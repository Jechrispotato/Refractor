...
Figure 19 outlines the chatbot interaction flow. The citizen will type a question, and the chatbot will perform a similarity search against the LGU's curated knowledge base using a Retrieval-Augmented Generation pipeline. If the similarity score is above the configured threshold, the chatbot will return the matching answer along with the source document. If the score is below the threshold, the chatbot will return a safe fallback message and offer to file a support ticket on behalf of the citizen.
...
Figure 32 shows a Use Case Diagram of the interactions between the Super Administrator actor and the system. The diagram presents the main functions: provisioning and deactivating LGU tenants, managing feature flags per LGU, monitoring cross-LGU analytics, supervising compliance and audit logs, and overseeing user accounts across all LGUs. It illustrates how the Super Administrator will interact with the platform to oversee the multi-tenant deployment of AGAPP.
...
| Add LGU button | Show Add LGU form | Allows the Super Admin to provision a new tenant municipality | Click | (To be tested) | Passed/Failed |
| Edit LGU button | Show Edit LGU form | Allows the Super Admin to edit tenant details | Click | (To be tested) | Passed/Failed |
| Deactivate LGU button | Show confirmation dialog | Deactivates an existing tenant LGU | Click | (To be tested) | Passed/Failed |
| Feature Flag toggle | Enable or disable a module per LGU | Controls which AGAPP modules are visible to each tenant | Click | (To be tested) | Passed/Failed |
...
| Ask question | Return retrieval-grounded answer or safe fallback | Performs the RAG search and returns an answer | Type and send | (To be tested) | Passed/Failed |
...
* **Maintainability:** Used to test the system's architecture, code quality, and ease of updates, ensuring long-term maintainability and scalability across additional LGU tenants.  
* **Portability:** Used to test AGAPP's ability to be deployed across different LGU IT infrastructures and to integrate with other government systems, ensuring smooth implementation when new municipalities are onboarded.
